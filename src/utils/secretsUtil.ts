import * as AWS from 'aws-sdk';

export const region = process.env.AWS_REGION || 'ap-southeast-1';

const client = new AWS.SecretsManager({
  region
});

export function getSecret(secretName = `${process.env.STAGE}/lando/main`): Promise<string> {
  return client.getSecretValue({ SecretId: secretName }).promise().then(data => data.SecretString);
}

export async function getJsonSecret(key: string, secretName = `${process.env.STAGE}/lando/main`): Promise<string> {
  return client.getSecretValue({ SecretId: secretName }).promise().then(data => JSON.parse(data.SecretString || '')[key]);
}
